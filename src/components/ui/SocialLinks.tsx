  ...
  return (
    ...
    >
      {socials.map((social, index) => (
        ...
        >
          <span className="block text-lg font-medium text-foreground/70 hover:text-accent transition-colors">{social.name}</span>
          <AnimatePresence>
            {hoveredSocial === social.name && (
              ...
              >
                <motion.img
                  ...
                  src={social.image}
                  alt={social.name}
                  className="w-12 h-12" // Reduced from w-16 h-16 to w-12 h-12
                  initial={{
                    y: -40,
                    rotate: rotation,
                    opacity: 0,
